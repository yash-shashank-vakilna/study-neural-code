...
for fi=3:length(fName)
    ...
    for regI = 1:5
        ...
        for chani = 1:length(filtMEA.mea.channel_names)
            [ histcounts] = computeWeightedPolarhistogramValues(spikeMEA.mea.spike_data{chani}, filtMEA.mea.filtered_data{chani},nbins, powerweight);
            allregionresults{fi}{regI}{chani,1} = histcounts;
            allregionresults{fi}{regI}{chani,2} = "binned polar histogram for for reg:" +regI+ " chani:"+filtMEA.mea.channel_names{chani};
