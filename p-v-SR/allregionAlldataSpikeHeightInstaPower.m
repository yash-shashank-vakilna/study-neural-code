    ...
    for regI = 1:5
        ...
        for chani = 1:length(filtMEA.mea.channel_names)
            ...
            if ~(isempty(spikeMEA.mea.spike_data{chani}))
                [ spikeHeight, spike_triggered_i_power] = computeSpikeheightvsPower...          %compute spike height and spike triggered 
                    (spikeMEA.mea.spike_data{chani}, zscore(filtMEA.mea.filtered_data{chani}));
                allregionresults{fi}{regI}{chani,1} = spikeHeight;
                allregionresults{fi}{regI}{chani,2} = spike_triggered_i_power;                  %spike triggered magnitude instantaneous power
