function h=plplot(x, xmin, alpha, makerType, displayName)
...
%         figure;
        h = loglog(c(:,1),c(:,2),makerType,'MarkerSize',4,'MarkerFaceColor',[1 1 1],'DisplayName',displayName); hold on;
        loglog(cf(:,1),cf(:,2),strcat('k','--'),'LineWidth',3,'MarkerEdgeColor','k'); hold off;
...
%         figure;
        h = loglog(c(:,1),c(:,2),makerType,'MarkerSize',8,'MarkerFaceColor',[1 1 1],'DisplayName',displayName); hold on;
        loglog(cf(:,1),cf(:,2),'k--','LineWidth',2,'HandleVisibility','off'); hold off;
        xr  = [10.^floor(log10(min(x))) 10.^ceil(log10(max(x)))];
        xrt = (round(log10(xr(1))):2:round(log10(xr(2))));
        if length(xrt)<4, xrt = (round(log10(xr(1))):1:round(log10(xr(2)))); end;
        yr  = [10.^floor(log10(1/n)) 1];
